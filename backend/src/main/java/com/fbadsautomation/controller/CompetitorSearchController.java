...
public class CompetitorSearchController {
    ...
    public ResponseEntity<ApiResponse<SearchResultResponse>> search(@RequestBody SearchRequest request,
        ...
        String engine = request.getEngine() != null ? request.getEngine() : "google";
        int limit = request.getLimit() != null ? request.getLimit() : 10;
        Map<String, String> options = new HashMap<>();
        options.put("location", request.getLocation());
        options.put("gl", request.getGl());
        options.put("hl", request.getHl());

        SearchApiResponse apiResponse = searchApiService.search(engine, request.getQuery(), options, limit);
        ...
        if (userId != null) {
            ...
            competitorService.recordSearchHistory(
                request.getQuery(),
                request.getGl() != null && !request.getGl().isBlank() ? request.getGl() : request.getLocation(),
                userId,
                engine.toUpperCase(),
                count,
                count > 0
